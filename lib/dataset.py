...
class ScannetQADataset(Dataset):
    ...
    def getitem_mixed(self, idx):
        other_idx = np.random.randint(0, len(self) - 1)

        start = time.time()
        scene_id = self.scanqa[idx]['scene_id']
        other_scene_id = self.scanqa[other_idx]['scene_id']

        if self.split != 'test':
            object_ids = self.scanqa[idx]['object_ids']
            object_names = [' '.join(object_name.split('_')) for object_name in
                            self.scanqa[other_idx]['object_names']]
        else:
            object_ids = None
            object_names = None
        ...
        # No merging of the language features.
        #
        ...
        mesh_vertices = np.concatenate([mesh_vertices, other_mesh_vertices], axis=0)
        instance_labels = np.concatenate([instance_labels, other_instance_labels], axis=0)
        semantic_labels = np.concatenate([semantic_labels, other_semantic_labels], axis=0)
        instance_bboxes = np.concatenate([instance_bboxes, other_instance_bboxes], axis=0)
    ...
    def __getitem__(self, idx):
        if np.random.uniform() < self.mixed_proba:
            return self.getitem_mixed(idx)
        else:
            return self.getitem_unmixed(idx)
